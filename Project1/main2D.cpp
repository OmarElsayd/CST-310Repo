...
struct Point {
  ...
  Point(GLfloat x = 0, GLfloat y = 0): x(x), y(y) {}  // Constructor
  Point midpoint(Point p) {return Point((x + p.x) / 2.0, (y + p.y) / 2.0);} // Midpoint
};
...
void display() {
  ...
  static Point p = vertices[0]; // Start with one of the vertices
  glBegin(GL_POINTS); // Begin plotting points
  for (int k = 0; k < 100000; k++) {  // Plot 100000 points
...
void init() { 
  ...
  glOrtho(0.0, 500.0, 0.0, 500.0, 0.0, 1.0);  // Set the viewing volume 
}
